...
- (id)init {
    ...
    if (self) {
        ...
        //TODO remove if not ok here
        //Add TreeView window here
        /*
        if ( self.treeController == nil ) {
            printf("Activating tree controller\n");
            self.treeController = [[NSWindowController alloc] initWithWindowNibName:@"TreeView"];
            [self.treeController showWindow: nil];
            self.treeWindow = [self.treeController window];
     
            if (! [self.treeController window] )
                printf("Failed initializing tree window\n");
        }
         */
            ...
            forObjectType:(iTermObjectType)objectType
{
    ...
    bool bookmarkCmdEqToString = [[ITAddressBookMgr bookmarkCommand:aDict
                                                      forObjectType:objectType] isEqualToString:@"$$"];
    
    bool objKeyCustomCommand = [[aDict objectForKey:KEY_CUSTOM_COMMAND] isEqualToString:@"Yes"];
    
    //TODO remove if better way
    objKeyCustomCommand = false;
    
    if (aDict == nil || bookmarkCmdEqToString || !objKeyCustomCommand) {
...
- (void)refreshSoftwareUpdateUserDefaults {
    ...
    //Do not check for anything
    //Do not check for test releases
    /*
    checkForTestReleases = FALSE;
