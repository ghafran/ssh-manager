...
typedef struct {
    BOOL antiAlias;           // Use anti-aliasing?
     __unsafe_unretained NSColor *color;           // Foreground color. Do not assign directly to this. Use CRunAttrsSetColor().
...
struct CRun {
    ...
    int index;                // -1 if nothing allocated, else start index of codes, glyphs, advances
     __unsafe_unretained NSString *string;         // If set then there are no codes or glyphs, but may be advances.
    int key;                  // For complex chars, this is the key that gives the sting.
    BOOL terminated;          // No more appends allowed (will go into |next|)
     __unsafe_unretained CRunStorage *storage;     // Backing store for codes, glyphs, and advances.
