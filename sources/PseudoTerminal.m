...
@implementation PseudoTerminal {
    ...
    IBOutlet NSWindow    *treeviewWindow;
    IBOutlet NSTextField *sendCommands;
...
- (IBAction)addfolder_button_clicked:(id)sender {
    printf("***** folder Window!");
    
    [ self.ssh_add_folder close ];
    
    if ([[self.ssh_add_folder title]  isEqual: @"Edit folder"])
    {
        [[NSNotificationCenter defaultCenter] postNotificationName:@"folder_edit_window_closed"
                                                        object:self];
    }
    else if ([[self.ssh_add_folder title]  isEqual: @"Add folder"])
    {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"folder_add_window_closed"
                                                            object:self];

    }
}
- (IBAction)ssh_button_clicked:(id)sender {

}
- (IBAction)performClose:(id)sender {
    printf("***** SSH Window!");
   
    if ([[self.ssh_window title]  isEqual: @"Add connection"])
    {

        [[NSNotificationCenter defaultCenter] postNotificationName:@"ssh_add_window_closed"
                                                        object:self];
    }
    else if ( [[self.ssh_window title]  isEqual: @"Edit connection"] )
    {
        
        [[NSNotificationCenter defaultCenter] postNotificationName:@"ssh_edit_window_closed"
                                                            object:self];
    }
    [self. ssh_window close];
}
...
- (id)init {
    ...
    printf("PseudoTerminal Initializing");
...
- (void)finishInitializationWithSmartLayout:(BOOL)smartLayout
    ...
    /*
    [[NSNotificationCenter defaultCenter] postNotificationName:@"PseudoTermWindowFinished" object: self];
    */
    

    windowType = WINDOW_TYPE_NO_TITLE_BAR;
    
    //TODO remove if/when finished
    //smartLayout = NO;
    printf("Smartlayout: %d\n", smartLayout);
    
    // Force the nib to load
    
    printf("finishInitalizationWithSmartLayouyt");
    ...
    switch (windowType) {
        ...
        case WINDOW_TYPE_NO_TITLE_BAR:
            ...
            if (haveScreenPreference_) {
                printf("Moving window to screen \n");
    ...
    printf("InitWithContentRect\n");
    myWindow = [[PTYWindow alloc] initWithContentRect:initialFrame
                                            styleMask:styleMask
                                              backing:NSBackingStoreBuffered
                                                defer:NO];
    printf("AFTER InitwithContentRect\n");
    
    [myWindow setShowsResizeIndicator: false];
   ...
   // [self->treeviewWindow addChildWindow: self.ssh_add_folder ordered:NSWindowAbove];
    ...
    [self->treeviewWindow setDelegate:self];
    ...
    //[[parentWindow window] addChildWindow: [self window] ordered: NSWindowAbove];
    //[[self window] addChildWindow: [parentWindow window] ordered: NSWindowBelow];
    ...
    if ([self shouldShowToolbelt]) {
        ...
        if (![self anyFullScreen]) {
            ...
            if (overage <= 0 && !NSEqualRects(self.window.frame, windowFrame)) {
                printf("Setting frame from toggleToolbeltVisibility");
                didResizeWindow = YES;
        ...
        printf("Show toolbelt\n");
        if (![self anyFullScreen] && hidingToolbeltShouldResizeWindow_) {
            NSRect windowFrame = self.window.frame;
            windowFrame.size.width -= toolbeltWidth_;
            didResizeWindow = YES;
            printf("window setFrame. toggleToolbeltVisibility\n");
            [self.window setFrame:windowFrame display:YES];
        }
    }
...
- (NSWindowController<iTermWindowController> *)terminalDraggedFromAnotherWindowAtPoint:(NSPoint)point
{
    ...
    printf("Terminal dragged from another window at point\n");
...
- (BOOL)broadcastInputToSession:(PTYSession *)session
{
    switch ([self broadcastMode]) {
        ...
        case BROADCAST_TO_ALL_TABS:
            printf("Broadcasting to all tabs\n");
            for (PTYTab *aTab in [self tabs]) {
                for (PTYSession* aSession in [aTab sessions]) {
                    if (aSession == session) {
                        return YES;
...
+ (NSWindowController*) parentWindow
{
    return nil;
}
...
{
    printf("bareTerminalWithArrangement\n");
    ...
    if (windowType == WINDOW_TYPE_TRADITIONAL_FULL_SCREEN) {
        ...
        rect = [term->treeviewWindow frame];
        ...
        printf("Setting frame from bareTerminalWithArrangement\n");
...
- (void) adjustChildWindowSize {
    ...
    rect.size.width = self->treeviewWindow.frame.size.width - 220;
    rect.size.height = self->treeviewWindow.frame.size.height - 90;
    
    rect.origin.x = self->treeviewWindow.frame.origin.x + 210;
    rect.origin.y = self->treeviewWindow.frame.origin.y + 5;
...
- (BOOL)loadArrangement:(NSDictionary *)arrangement sessions:(NSArray *)sessions
{
    ...
    printf("loadArrangement arrangement\n");
...
- (void)makeCurrentSessionFirstResponder {
   // [self->treeviewWindow setInitialFirstResponder: self->treeviewWindow];
   // [[self window] setInitialFirstResponder: self->treeviewWindow];

    /*
    printf("Making first responder\n");
...
- (void)canonicalizeWindowFrame {
    ...
    printf("canonicalizedWindowFrame");
    PTYSession* session = [self currentSession];
    ...
    switch (windowType_) {
        ...
        case WINDOW_TYPE_TOP:
            ...
            if (frame.size.width > 0) {
                printf("Canonicalize window frame\n");
...
- (void)windowDidResize:(NSNotification *)aNotification
{
    ...
    NSWindow *win = ( NSWindow*) aNotification.object;
    ...
    if ([win.title  isEqual: @"TreeView1"])
    {
        ...
        printf("Exiting, win title is %s\n", winTitle);
        return;
    }
    
 
    
  // printf("Detected resize, window name: ")

    PtyLog(@"windowDidResize to: %fx%f", [[self window] frame].size.width, [[self window] frame].size.height);
    printf("windowDidResize to: %fx%f", [[self window] frame].size.width, [[self window] frame].size.height);
...
{
    [SessionView windowDidResize];
    printf("Toggle tradditional full screen mode\n");
...
{
    printf("windowWillUseStandardFrame\n");
...
{
    printf("BOOL checking for sendInputToAllSessions...\n");
    ...
    if ([TABVIEW indexOfTabViewItemWithIdentifier:aTab] == NSNotFound) {
        ...
        if (self.windowInitialized && !_fullScreen) {
           // [[self window] makeKeyAndOrderFront:self];
            [self->treeviewWindow makeKeyAndOrderFront:self];
        } else {
